...
fn archives_from_before_recompression_still_extract() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let mut checked = 0;
    for name in ["legacy-max.narc", "legacy-normal.narc", "legacy-ppmd.narc"] {
...
fn deflate_containers_are_recompressed_and_still_extract() {
    ...
    for c in a.manifest.chunks.iter().filter(|c| c.filter == 34) {
...
/// takes 0.20% off them and narc takes 19.2%.
///
...
/// data. Measured on 7.27 MB of technical documentation: narc max 6,394,652 →
...
fn pdf_flate_streams_are_recompressed_and_still_extract() {
    ...
    assert_eq!(
        narc_core::deflate::find_streams(&pdf).len(),
        streams,
    ...
    let arc = tmp.path().join("pdf.narc");
    let mut a = Archive::create(&arc).unwrap();
    let stats = a
        .add_paths(std::slice::from_ref(&src), &PackOptions::new(Tier::Max))
        .unwrap();
