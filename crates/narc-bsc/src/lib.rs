...
//! Rust API.
//!
//! Why it is here at all: measured on this machine at narc's OWN 32 MiB unit,
...
//! (13.5 MB against narc's 9.3 MB), which is exactly why it joins the per-unit
...
/// deliberately absent: narc runs one worker per unit already, and letting the
...
/// concurrent compression and decompression from narc's worker threads is safe.
...
/// The largest block this codec accepts. libbsc counts in `int`, and narc's
