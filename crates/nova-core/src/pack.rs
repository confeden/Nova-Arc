...
//! ~4 MiB chunks, and that alone made nova 2.3x worse than 7-Zip on a set of
...
/// Per-unit log, enabled by `NOVA_UNIT_TRACE=<path>` and absent otherwise.
///
...
impl Trace {
    fn open() -> Option<Trace> {
        let path = std::env::var_os("NOVA_UNIT_TRACE")?;
