...
//!
//!   cargo run --release -p nova-core --example progress_probe -- <dir> [tier]

use std::path::PathBuf;
use std::sync::Mutex;
use std::time::Instant;

use nova_core::{Archive, PackOptions, Phase, Progress, Tier};
...
fn main() -> anyhow::Result<()> {
    ...
    let out = PathBuf::from(format!("test/progress-probe-{tier:?}.nva").to_lowercase());
