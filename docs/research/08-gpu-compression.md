...
Research for Nova Prism (`.nva` format: append-only, CDC chunks 256 KiB–4 MiB, blake3 dedup, zstd baseline).
...
- **The cross-vendor story improved dramatically in 2025–2026**: DirectStorage 1.4 (GDC 2026) added **Zstandard GPU decompression with an open-source (MIT) compute shader**, tuned for chunks ≤ 256 KB — almost exactly nova's chunk regime. Vulkan got `VK_EXT_memory_decompression` (GDeflate, multi-vendor). These are *decompression-only* paths.
...
| APIs | C, C++, Python; low-level **batched API** (many independent chunks — matches nova's chunk model) and high-level API (adds nvCOMP's own framing header) |
...
| **zstd** | yes | yes | **yes (standard frames)** | nova's baseline codec; ~16 GB/s compress reported on Blackwell; decomp optimized 2.2× on H100 |
...
Key interop fact for nova: the standard-format codecs used through the **low-level batched API** produce/consume standard bitstreams per chunk, so **a `.nva` written with GPU zstd remains extractable by pure-CPU zstd on any machine** (and vice versa: CPU-written chunks can be batch-decompressed on GPU). This is the property that lets GPU stay an optional accelerator rather than a format dependency. (Precedent: [zarr's GPU zstd codec](https://github.com/zarr-developers/zarr-python/pull/2863) decodes CPU-written zstd with nvCOMP.)
...
Interpretation for nova on an RTX 5060 Ti:
...
- Chunk-size sensitivity is high (thesis + Voltron Data tuning experiments): smaller chunks → more parallelism → higher throughput but worse ratio. nova's 256 KiB–4 MiB CDC chunks sit in the sweet band; nvCOMP internally likes 64–512 KiB (DE benchmarks use exactly 64 KiB/512 KiB).
...
- **Practical consequences for Nova Prism**:
...
- **Adoption reality**: weak in games (a handful of titles; GPU decompression steals frame-time). Irrelevant for nova — an archiver has no frame budget to protect, the machine is otherwise idle-ish, and freeing the CPU is a stated goal.
- **Limits for nova**: DirectStorage is **decompression-only** on GPU, D3D12/Windows-only, and its API shape is asset-streaming (file → GPU resource), not host-buffer → host-buffer. The valuable artifacts for nova are the **formats and the open shaders** (GDeflate HLSL, Zstd compute shader), not the runtime.
...
| Project | What | Perf claim | License | Liveness (verified) | Verdict for nova |
|---|---|---|---|---|---|
...
| [libcubwt](https://github.com/IlyaGrebnov/libcubwt) | GPU suffix array + BWT construction | GPU BWT for bsc `-G` mode | **Apache-2.0** | **active** (last push 2025-08-14) | only if nova ever grows a BWT codec; author (Grebnov) also maintains libsais/bsc |
...
| [cust](https://github.com/rust-gpu/rust-cuda) (Rust-CUDA) | write GPU kernels in Rust (NVVM backend) | rebooted 2025-01 after 3 years dormant; self-described early, **pinned to a specific nightly** | not production-ready; avoid for nova core |
...
| nvCOMP bindings | FFI to closed lib | **no mature crate.** `baracuda-nvcomp`/`-sys` (0.0.1-alpha, 2026-08-15) is scaffolding with a dynamic loader; `s4-codec` (1.5.2, 2026-07) demonstrates exactly the pluggable "nvCOMP zstd/Bitcomp GPU + CPU zstd fallback" pattern but is niche (<1 k downloads) | **write our own `nova-nvcomp-sys`**: bindgen over `nvcomp.h` batched C API + `libloading` for `nvcomp.dll`; ~9 codec modules but nova needs only `nvcompBatched{Zstd,LZ4}{Compress,Decompress}*` — a small, stable surface |
...
**Wins**
1. **Batch decompression of independent chunks** — nova's extract path is embarrassingly parallel over chunks; GPU decompression at 5–20+ GB/s exceeds any consumer NVMe, while leaving the CPU nearly idle (directly serves the "low priority, responsive Windows" requirement).
...
3. **Batch entropy coding** — ANS at hundreds of GB/s (dietgpu/nvCOMP) is the single largest GPU advantage; a future nova-specific filter+entropy stage could exploit it.
...
4. **8 GB VRAM under real desktop load**: browser + apps commonly hold 1–3 GB; nova must budget VRAM explicitly (see roadmap) and degrade to CPU on allocation failure.
...
## 9. Realistic GPU roadmap for Nova Prism
...
- `.nva` records only the codec ID (zstd/lz4) per chunk — **standard bitstreams only** (nvCOMP's proprietary ANS/Bitcomp/Cascaded formats are banned from the format for now; a weak PC without any GPU must always be able to extract with CPU zstd).
...
- `nova-nvcomp-sys` (bindgen + libloading) covering batched zstd + LZ4, compress and decompress; cudarc 0.19 for streams/memory/pinned buffers.
...
- Port a **decompressor** to WGSL: start from Microsoft's MIT Zstd compute shader (tuned for ≤ 256 KB chunks — matches nova) and/or GDeflate HLSL. Requires wgpu subgroups (native-only, fine for us).
...
**Phase 4 — research track (optional, nova-experimental codec ID)**
