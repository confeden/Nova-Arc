# 04 — FreeArc Lessons: Filter-Based Compression Architecture for nova
...
faster). The mechanism, not the codecs, is what nova should copy.
...
(preflate-rs, lepton_jpeg_rust). nova should reimplement the architecture, not port the code
...
Observations that matter for nova:
...
Weakness to fix in nova: grouping was **extension-based**, only partially content-based (planned
for 0.50, never shipped; misrouted DDS textures through the exe filter — see dispack below).
nova's phase-1 analyzer must detect by content (magic bytes + entropy/stride probes + PE/ELF
...
| **delta** | filter | auto-detects tables/records in binary data (stride 2/4/8…), subtracts columns | +5–30% on DB tables, structured binaries | fast | 7-Zip `Delta:{N}` (fixed stride, manual); kanzi has none as auto — **auto-stride detection is a differentiator nova should build** |
...
| **4x4** | wrapper | splits stream into blocks, compresses N blocks in parallel with any inner method (`4x4:lzma:...`); also the `#$compressed` MT trick | ~linear MT scaling at small ratio loss | — | standard block-parallel design (zstdmt, xz -T, 7-Zip); nova gets it naturally from chunk groups |
...
5. **Smart solid updates**: only changed solid blocks recompressed — the same requirement nova's
...
precisely to trade that back. nova's default tier should stay decompression-fast; only the max
...
| FAZip (standalone filter/codec CLI) | v0.3, Dec 2013 | GPL | **Dead.** Concept (expose each filter as standalone tool) worth copying for nova debugging |
...
| srep | frozen ~2014 | — | Superseded for nova by CDC dedup + rep |
...
**License consequence for nova:** FreeArc code (GPL-2.0, Haskell/C++) must not be linked or
translated line-by-line if nova wants MIT/Apache dual licensing. Chains, parameters, and
...
**Decision for nova:** implement BCJ-class rel→abs converters per ISA (x86, x86-64, ARM64;
...
FreeArc's answer was routing (`$compressed` → cheap rep+tornado) plus *external* precomp. nova
can do the same natively with maintained Rust libraries:

| Input | Tool (2026) | Gain | Cost | Notes |
|---|---|---|---|---|
| deflate streams (zip, gz, png, docx/xlsx, pdf) | **preflate-rs** 0.7.x | recompressed with LZMA/zstd afterwards: typically 20–50% vs storing the deflate stream | high CPU, but streamable; corrections-based model | Rust, Apache-2.0, Microsoft-maintained. Feed *expanded* data through the normal nova pipeline, store correction stream |
...
the dedup layer. For nova this means recompression transforms run in phase 1/ingest, before CDC
...
## 7. Deliverable: prioritized filter list for nova's two-phase analyzer
...
1. **rep / long-range matcher within solid blocks** (below CDC granularity, window ≥ codec dict).
   ...
   Implementation: rolling-hash long-match LZ à la rep/zstd-LDM. (nova's CDC dedup already
...
8. **Strong text codec tier**: PPMd-class or BWT (libbsc, Apache-2.0) for `$text` at max. RAR
   dropped PPMd for speed; nova can keep it optional behind the asymmetric-tier warning.
...
ingest:    detect → [format expansion: preflate / lepton / packMP3]   (phase 1)
           → CDC chunking + cross-file dedup (nova core)
...
- Known-good user chain (community folklore, Martin Ankerl):
  ...
  the same power in nova's config
